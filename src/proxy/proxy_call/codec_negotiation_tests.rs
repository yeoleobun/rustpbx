...
fn build_caller_answer_codec_info_for_test(
    ...
    append_dtmf_codecs_for_test(&mut codec_info, caller_dtmf_codecs, &mut used_payload_types);
...
fn test_should_advertise_caller_dtmf_for_media_proxy_even_without_callee_dtmf() {
    assert!(caller_negotiation::should_advertise_caller_dtmf(
        true, false
    ));
...
fn test_early_media_codec_extraction() {
    ...
    let wrong_codec_b = MediaNegotiator::select_best_codec(&wrong_codecs, &allow_codecs).unwrap();
    assert_ne!(
